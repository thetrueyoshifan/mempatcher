...
auto resolve_address(std::uint8_t* base, const parser::patch& patch) -> std::uint8_t*
{
    ...
    if (patch.type == parser::addr_type::file)
    {
        ...
        if (!result)
        {
            spdlog::error("Failed to convert {} on line {} from 0x{:X} to RVA",
                patch.type_name(), patch.line, patch.address);
...
auto compare_data(std::uint8_t* target, const parser::patch& patch)
{
    ...
    {
        spdlog::error("Failed to validate data on line {} at {} 0x{:X}",
            patch.line, patch.type_name(), patch.address);
...
auto write_data(std::uint8_t* target, const parser::patch& patch)
{
    ...
    {
        spdlog::error("Failed to change memory protection for patch on line {} at {} 0x{:X}",
            patch.line, patch.type_name(), patch.address);
    ...
    {
        spdlog::error("Failed to restore memory protection for patch on line {} at {} 0x{:X}",
            patch.line, patch.type_name(), patch.address);

        return false;
    }

    auto line = fmt::format("Applied patch on line {} at {} {}",
...
auto patch::apply(std::uint8_t* base, const parser::patch& patch) -> bool
{
    ...
    if (patch.on.empty())
    {
        spdlog::info("Validated data on line {} at {} {} from 0x{:X}",
            patch.line, patch.type_name(), patch.target_name(),
